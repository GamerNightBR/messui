...
int MameUIVerifyRomSet(int game, bool choice)
{
	...
	if (choice)
		auditor.messui_summarize(driver_list::driver(game).name, &summary_string); // audit all games
	else
...
static DWORD WINAPI AuditThreadProc(LPVOID hDlg)
{
	...
	while (!bCancel)
	{
		if (!bPaused)
		{
			if (rom_index != -1)
			{
				sprintf(buffer, "Checking System %s - %s",
...
// not sure if this actually works, as we currently have no samples in MESS
