//============================================================
//
//  menu.c - This is the NEWUI Windows dropdown menu system
//
//============================================================
...
static dialog_box *build_option_dialog(device_image_interface *dev, char *filter, size_t filter_len, int *create_format, option_resolution **create_args)
{
	...
	// make the filter
	pos = 0;
	for (format = dev->device_get_creatable_formats(); format; format = format->m_next)
...
static void change_device(HWND wnd, device_image_interface *image, int is_save)
{
	...
	if (is_save
		...
		&& (image->device_get_creatable_formats()))
	{
...
static void prepare_menus(HWND wnd)
{
	...
	set_command_state(menu_bar, ID_KEYBOARD_NATURAL, (has_keyboard && window->machine().ioport().natkeyboard().can_post()) ? (ui_get_use_natural_keyboard(window->machine()) ? MFS_CHECKED : MFS_ENABLED): MFS_GRAYED);
...
static void win_toggle_menubar(void)
{
	...
	for (window = win_window_list; window; window = window->next)
	{
...
static BOOL win_check_option_control(HWND control)
{
	BOOL rc = FALSE;
	TCHAR class_name[32];
...
static BOOL win_adjust_option_control(HWND control, int delta)
{
	const option_guide *guide;
	const char *optspec;
	struct OptionRange ranges[128];
...
static optreserr_t win_add_resolution_parameter(HWND control, option_resolution *resolution)
{
	const option_guide *guide;
	char buf[256];
	optreserr_t err;
...
static int win_dialog_add_slider(dialog_box *dialog, const char *item_label, int default_value,
	int min_value, int max_value, dialog_itemstoreval storeval, void *storeval_param)
{
	short x;
	short y;
...
static int win_dialog_add_image(dialog_box *dialog, const png_info *png)
{
	WORD id;
	HBITMAP bitmap;
	short x, y;
...
static int win_dialog_add_separator(dialog_box *dialog)
{
...
static char *win_dialog_strdup(dialog_box *dialog, const char *s)
{
